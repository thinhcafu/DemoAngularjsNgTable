var app = angular.module('myApp', ['ui.grid', 'ui.grid.selection', 'smart-table', 'ngTable']);
app.controller('productCtrl', ['$scope', '$http', '$filter', 'uiGridConstants', 'ngTableParams', function ($scope, $http, $filter, uiGridConstants, ngTableParams) {
    $scope.lastId = 0;
    $scope.products = [];
    ...
    $scope.tableParams = new ngTableParams({
        ...
        total: $scope.dataProduct.length,
        getData: function ($defer, params) {
            var orderedData = params.filter() ?
                $filter('filter')($scope.dataProduct, params.filter()) :
                $scope.dataProduct;

            $scope.sanpham = orderedData.slice((params.page() - 1) * params.count(), params.page() * params.count());

            params.total(orderedData.length);
            $defer.resolve($scope.sanpham);
        }
    ...
    $scope.loadProducts = function (page) {
        $.ajax({
            ...
            $scope.$apply(function () {
                ...
                $scope.dataProduct = $scope.products;
